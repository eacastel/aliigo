"use strict";(()=>{function z(o){if(!Array.isArray(o))return;let l=[];for(let e of o){if(!e||typeof e!="object")continue;let t=e,s=t.type;if(s==="cta"){let i=typeof t.label=="string"?t.label:"",n=typeof t.url=="string"?t.url:"";i&&n&&l.push({type:"link",label:i,url:n});continue}if(s==="collect_lead"){let i=Array.isArray(t.fields)?t.fields.filter(r=>r==="name"||r==="email"||r==="phone"):[],n=typeof t.reason=="string"?t.reason:void 0;l.push({type:"lead_form",fields:i.length?i:["name","email"],reason:n});continue}}return l.length?l:void 0}var J={en:{pill:o=>o?`Ask ${o}`:"Chat",title:o=>o?`${o} Assistant`:"Assistant",welcome:"Ask a question and we\u2019ll help right away.",placeholder:"Type your question\u2026",send:"Send",lead:{title:"Share your details",name:"Name",email:"Email",phone:"Phone",submit:"Send details",sent:"Thanks! We\u2019ll be in touch.",message:"Here are my contact details.",hiddenMessage:"Lead submitted.",followUp:"Thanks \u2014 I\u2019ve got your details. Do you have any other questions? I can share pricing, show how the widget works, or help you get set up.",consent:"I agree to be contacted about my request.",consentNote:"Your information will only be used to follow up on this request."}},es:{pill:o=>o?`Pregunta a ${o}`:"Chat",title:o=>o?`Asistente de ${o}`:"Asistente",welcome:"Haz tu consulta y te ayudamos al momento.",placeholder:"Escribe tu consulta\u2026",send:"Enviar",lead:{title:"D\xE9janos tus datos",name:"Nombre",email:"Email",phone:"Tel\xE9fono",submit:"Enviar datos",sent:"\xA1Gracias! Te contactaremos.",message:"Aqu\xED tienes mis datos de contacto.",hiddenMessage:"Datos enviados.",followUp:"Gracias \u2014 ya tengo tus datos. \xBFTienes alguna otra pregunta? Puedo compartir precios, ense\xF1arte c\xF3mo funciona el widget o ayudarte a empezar.",consent:"Acepto que me contacten sobre mi solicitud.",consentNote:"Usaremos estos datos solo para dar seguimiento a tu solicitud."}}};function X(o){return(o||"").toLowerCase().startsWith("es")}function P(o,l){let t=(o||"").trim().match(/#([0-9a-fA-F]{3}){1,2}/g)||[],s=t[0]||(l==null?void 0:l.bg)||"#111827",i=t[1]||(l==null?void 0:l.text)||"#ffffff";return{bg:s,text:i}}function U(o){return Math.max(0,Math.min(1,o))}function Q(o){let l=(o||"").trim().replace("#","");if(l.length===3){let e=parseInt(l[0]+l[0],16),t=parseInt(l[1]+l[1],16),s=parseInt(l[2]+l[2],16);return{r:e,g:t,b:s}}if(l.length===6){let e=parseInt(l.slice(0,2),16),t=parseInt(l.slice(2,4),16),s=parseInt(l.slice(4,6),16);return{r:e,g:t,b:s}}return null}function Z(o,l){let e=Q(o);return e?`rgba(${e.r}, ${e.g}, ${e.b}, ${U(l)})`:null}var F=class extends HTMLElement{constructor(){super(...arguments);this.STORAGE_TTL_MS=1800*1e3;this.STORAGE_PREFIX="aliigo_widget_v1";this.pendingScroll=null;this.expiryTimer=null;this.lastActiveAtMs=null;this.lastRenderOpen=!1;this.pendingFocus=!1;this.sessionHydrated=!1;this.cachedTheme=null;this.cachedBrand="";this.onFocus=()=>this.checkExpiryNow();this.onVis=()=>{document.hidden||this.checkExpiryNow()};this.state={open:!1,busy:!1,conversationId:null,msgs:[],session:null,locale:"en",visitorSessionId:null}}static get observedAttributes(){return["variant","embed-key","api-base","locale","session-token","floating-mode","theme","brand","start-open"]}ensureRoot(){this.shadowRoot?this.root=this.shadowRoot:this.root=this.attachShadow({mode:"open"})}connectedCallback(){if(this.ensureRoot(),this.state.visitorSessionId=this.getOrCreateVisitorSessionId(),this.loadPersisted(),this.getVariant()==="floating"&&this.state.msgs.length>0&&(this.state.open=!0,this.pendingScroll="bottom",this.pendingFocus=!0),this.getVariant()==="floating"&&this.getFloatingMode()==="fixed"){let e=document.body;this.parentElement!==e&&e.appendChild(this)}this.getVariant()==="floating"&&this.getStartOpen()&&(this.state.open=!0,this.pendingScroll="bottom",this.pendingFocus=!0),window.addEventListener("focus",this.onFocus),document.addEventListener("visibilitychange",this.onVis),this.scheduleExpiryTimer(),this.sessionHydrated=!1,this.render(),this.ensureSession().finally(()=>{this.sessionHydrated=!0,(this.state.session||!this.getEmbedKey())&&this.render()})}disconnectedCallback(){window.removeEventListener("focus",this.onFocus),document.removeEventListener("visibilitychange",this.onVis),this.clearExpiryTimer()}attributeChangedCallback(e,t,s){t!==s&&(this.ensureRoot(),this.render(),(e==="embed-key"||e==="api-base"||e==="session-token")&&(this.state.session=null,(e==="embed-key"||e==="session-token")&&(this.cachedTheme=null,this.cachedBrand="",this.sessionHydrated=!1),this.ensureSession().finally(()=>{this.sessionHydrated=!0,(this.state.session||!this.getEmbedKey())&&this.render()})))}clearExpiryTimer(){this.expiryTimer!=null&&(window.clearTimeout(this.expiryTimer),this.expiryTimer=null)}scheduleExpiryTimer(){if(this.clearExpiryTimer(),!this.lastActiveAtMs||this.state.msgs.length===0)return;let e=this.lastActiveAtMs+this.STORAGE_TTL_MS-Date.now();if(e<=0){this.expireConversation();return}this.expiryTimer=window.setTimeout(()=>{this.checkExpiryNow()},e+50)}checkExpiryNow(){if(!this.lastActiveAtMs||this.state.msgs.length===0)return;Date.now()-this.lastActiveAtMs>=this.STORAGE_TTL_MS?this.expireConversation():this.scheduleExpiryTimer()}expireConversation(){this.clearPersisted(),this.lastActiveAtMs=null,this.clearExpiryTimer(),this.state.msgs=[],this.state.conversationId=null,this.state.busy=!1,this.getVariant()==="floating"&&(this.state.open=!1),this.render()}applyPendingScroll(){let e=this.root.querySelector(".messages");if(!e||!this.pendingScroll)return;let t=this.pendingScroll;this.pendingScroll=null,t==="bottom"&&(e.scrollTop=e.scrollHeight),requestAnimationFrame(()=>{var i;let s=Math.max(0,e.scrollHeight-e.clientHeight);if(t==="bottom"){e.scrollTop=s;return}for(let n=this.state.msgs.length-1;n>=0;n--)if(((i=this.state.msgs[n])==null?void 0:i.role)==="assistant"){let r=this.root.querySelector(`#msg-${n}`);if(!r)return;let u=r.offsetTop-12;e.scrollTop=Math.max(0,Math.min(u,s));return}})}applyPendingFocus(){this.pendingFocus&&(this.pendingFocus=!1,requestAnimationFrame(()=>{let e=this.root.querySelector(".input");e==null||e.focus({preventScroll:!0})}))}getOrCreateVisitorSessionId(){let e="aliigo_visitor_session_v1";try{let t=localStorage.getItem(e);if(t&&t.length>=24)return t;let s=crypto.getRandomValues(new Uint8Array(16)),i=Array.from(s).map(n=>n.toString(16).padStart(2,"0")).join("");return localStorage.setItem(e,i),i}catch{return`${Date.now()}_${Math.random().toString(16).slice(2)}`}}storageKey(){let e=this.getEmbedKey(),t=this.getSessionTokenOverride(),s=(window.location.hostname||"").toLowerCase(),i=e||t||"no-key";return`${this.STORAGE_PREFIX}:${i}:${s}`}loadPersisted(){try{let e=localStorage.getItem(this.storageKey());if(!e)return;let t=JSON.parse(e);t.theme&&(this.cachedTheme=t.theme),typeof t.brand=="string"&&(this.cachedBrand=t.brand);let s=typeof t.lastActiveAt=="number"?t.lastActiveAt:typeof t.savedAt=="number"?t.savedAt:0;if(!s||Date.now()-s>this.STORAGE_TTL_MS){this.clearPersisted();return}typeof t.open=="boolean"&&(this.state.open=t.open),Array.isArray(t.msgs)&&(this.state.msgs=t.msgs),typeof t.conversationId=="string"&&(this.state.conversationId=t.conversationId),(t.locale==="en"||t.locale==="es")&&(this.state.locale=t.locale),this.state.msgs.length>0&&(this.lastActiveAtMs=s||Date.now())}catch{}}savePersisted(e=!0){try{let t=this.storageKey(),s=Date.now(),i=s,n;try{let h=localStorage.getItem(t);if(h){let a=JSON.parse(h);typeof a.savedAt=="number"&&(i=a.savedAt),typeof a.lastActiveAt=="number"&&(n=a.lastActiveAt)}}catch{}let r=e?s:n,u={savedAt:i,lastActiveAt:r,conversationId:this.state.conversationId,msgs:this.state.msgs,locale:this.state.locale,open:this.state.open,theme:this.cachedTheme,brand:this.cachedBrand};localStorage.setItem(t,JSON.stringify(u)),this.state.msgs.length>0?this.lastActiveAtMs=e?s:r!=null?r:this.lastActiveAtMs:this.lastActiveAtMs=null,this.scheduleExpiryTimer()}catch{}}clearPersisted(){try{localStorage.removeItem(this.storageKey())}catch{}}getBrandOverride(){return(this.getAttribute("brand")||"").trim()||null}getVariant(){let e=(this.getAttribute("variant")||"floating").toLowerCase();return e==="inline"||e==="hero"?e:"floating"}getEmbedKey(){return(this.getAttribute("embed-key")||"").trim()}getApiBase(){return(this.getAttribute("api-base")||"https://aliigo.com").trim().replace(/\/$/,"")}getLocaleOverride(){let e=(this.getAttribute("locale")||"").trim().toLowerCase();return e?e.startsWith("es")?"es":"en":null}getSessionTokenOverride(){return(this.getAttribute("session-token")||"").trim()||null}getThemeOverride(){let e=(this.getAttribute("theme")||"").trim();if(!e)return null;try{let t=JSON.parse(e),s={};return typeof t.headerBg=="string"&&(s.headerBg=t.headerBg),typeof t.bubbleUser=="string"&&(s.bubbleUser=t.bubbleUser),typeof t.bubbleBot=="string"&&(s.bubbleBot=t.bubbleBot),typeof t.sendBg=="string"&&(s.sendBg=t.sendBg),typeof t.panelBg=="string"&&(s.panelBg=t.panelBg),typeof t.panelOpacity=="number"&&(s.panelOpacity=t.panelOpacity),s}catch{return null}}getStartOpen(){return(this.getAttribute("start-open")||"").toLowerCase()==="true"}getHideHeader(){return(this.getAttribute("hide-header")||"").toLowerCase()==="true"}getFloatingMode(){return(this.getAttribute("floating-mode")||"").toLowerCase()==="absolute"?"absolute":"fixed"}async ensureSession(){let e=this.getSessionTokenOverride(),t=this.getEmbedKey();if(!t&&!e)return null;if(this.state.session&&!e)return this.state.session;try{let s=this.getApiBase();if(e){let a=this.getLocaleOverride(),m=this.getThemeOverride(),p=this.getBrandOverride();return this.state.session={token:e,locale:a||this.state.locale,brand:(p||"").trim(),slug:"",theme:m},this.cachedTheme=this.state.session.theme||null,this.cachedBrand=this.state.session.brand||"",this.state.locale=a||this.state.locale,this.savePersisted(!1),this.render(),this.state.session}let i=new URL(`${s}/api/embed/session`);i.searchParams.set("key",t),i.searchParams.set("host",window.location.hostname);let n=await fetch(i.toString(),{method:"GET"}),r=await n.json().catch(()=>({}));if(!n.ok||!r.token)return this.state.session=null,this.renderError(r.error||"Session error"),null;let h=this.getLocaleOverride()||(X(r.locale||"en")?"es":"en");return this.state.session={token:r.token,locale:h,brand:(r.brand||"").trim(),slug:(r.slug||"").trim(),theme:r.theme||null},this.cachedTheme=this.state.session.theme||null,this.cachedBrand=this.state.session.brand||"",this.state.locale=h,this.savePersisted(!1),this.render(),this.state.session}catch{return this.renderError("Network error"),null}}async tryRefreshSessionToken(){var t;this.state.session=null;let e=await this.ensureSession();return(t=e==null?void 0:e.token)!=null?t:null}renderError(e){this.ensureRoot(),this.root.innerHTML=`
    ...
    `}render(){var q;this.ensureRoot();let e=this.getVariant();if(!!this.getEmbedKey()&&!this.getThemeOverride()&&!this.cachedTheme&&!((q=this.state.session)!=null&&q.theme)&&!this.sessionHydrated){let b=e==="floating"?`wrap floating ${this.getFloatingMode()}`:e==="hero"?"wrap hero":"wrap inline",d=e==="hero"?"panel hero":e==="inline"?"panel inline":"panel";this.root.innerHTML=`
        ...
        `;return}let s=this.state.session,i=this.getLocaleOverride()||this.state.locale,n=J[i],r=this.getThemeOverride()||(s==null?void 0:s.theme)||this.cachedTheme||{},u=(r.panelBg||"").trim(),h=typeof r.panelOpacity=="number"?U(r.panelOpacity):1,a=u?Z(u,h):null,m=a?`style="--panel-bg:${a};"`:"",p=(this.getBrandOverride()||(s==null?void 0:s.brand)||this.cachedBrand||"").trim(),c=P(r.headerBg,{bg:"#111827",text:"#ffffff"}),g=P(r.bubbleUser,{bg:"#2563eb",text:"#ffffff"}),v=P(r.bubbleBot,{bg:"#f3f4f6",text:"#111827"}),y=P(r.sendBg,{bg:"#2563eb",text:"#ffffff"}),I=this.getHideHeader(),L=e!=="floating"?!0:this.state.open,C=L,D=C&&!this.lastRenderOpen;this.lastRenderOpen=C;let K=this.getFloatingMode(),_=e==="floating"?`wrap floating ${K}`:e==="hero"?"wrap hero":"wrap inline",N=e==="hero"?"panel hero":e==="inline"?"panel inline":"panel",j=D?`${N} animate-in`:N,V=I?`${j} no-header`:j,B=this.state.msgs,W=B.length===0?`<div class="row bot" id="msg-0">
            <div class="bubble bot anim" style="--bg:${v.bg};--fg:${v.text};background:var(--bg);color:var(--fg);">
              ${n.welcome}
            </div>
          </div>`:B.map((b,d)=>{let w=b.role==="user",$=d===B.length-1,A=w?`--bg:${g.bg};--fg:${g.text};background:var(--bg);color:var(--fg);`:`--bg:${v.bg};--fg:${v.text};background:var(--bg);color:var(--fg);`;return`<div class="row ${w?"user":"bot"}" id="msg-${d}">
                <div class="bubble ${w?"user":"bot"} ${$?"anim":""}" style="${A}">
                  ${te(b.content)}
                  ${!w&&Array.isArray(b.actions)&&b.actions.length?`<div class="actions">
                          ${b.actions.map(f=>{if(f.type==="link")return`<a class="action" href="${x(f.url)}" target="_blank" rel="noopener noreferrer">${x(f.label)}</a>`;if(f.type==="handoff")return`<button class="action-btn" type="button" data-action="handoff" data-i="${d}">${x(f.label)}</button>`;if(f.type==="lead_form"){let E=Array.isArray(f.fields)?f.fields:["name","email"],M=f.reason?`<div class="lead-reason">${x(f.reason)}</div>`:"",S=E.map(k=>{let G=k==="name"?n.lead.name:k==="phone"?n.lead.phone:n.lead.email,Y=k==="email"?"email":k==="phone"?"tel":"text";return`<label class="lead-field">
                                      <span class="lead-label">${x(G)}</span>
                                      <input class="lead-input" name="${k}" type="${Y}" required />
                                    </label>`}).join(""),O=`<label class="lead-consent">
                                  <input type="checkbox" name="consent" required />
                                  <span>${x(n.lead.consent)}</span>
                                </label>
                                <div class="lead-consent-note">${x(n.lead.consentNote)}</div>`;return`<form class="lead-form" data-action="lead-form" data-i="${d}">
                                  ${M}
                                  ${S}
                                  ${O}
                                  <button class="lead-submit" type="submit">${x(n.lead.submit)}</button>
                                </form>`}return""}).join("")}
                        </div>`:""}
                </div>
              </div>`}).join("");if(e==="floating"&&!L){if(this.lastRenderOpen=!1,!(!!(s!=null&&s.token)||!!this.getSessionTokenOverride())){this.root.innerHTML=`<style>${this.css()}</style>`;return}this.root.innerHTML=`
        <style>${this.css()}</style>
        <div class="${_}">
          <button class="pill" style="background:${y.bg};color:${y.text};">${n.pill(p)}</button>
        </div>
      `,this.root.querySelectorAll(".action-btn[data-action='handoff']").forEach(w=>{w.addEventListener("click",()=>{this.send("I\u2019d like a human follow-up.")})});let d=this.root.querySelector(".pill");d==null||d.addEventListener("click",()=>{this.state.open=!0,this.pendingFocus=!0,this.savePersisted(!1),this.pendingScroll="bottom",this.render()});return}this.root.innerHTML=`
      <style>${this.css()}</style>
      <div class="${_}">
      <div class="${V}" ${m}>
          ...
          <form class="form">
            ...
            <button class="send" type="submit" style="background:${y.bg};color:${y.text};" ${this.state.busy||!(s!=null&&s.token)?"disabled":""}>${n.send}</button>
          </form>
        </div>
      </div>
    `;let H=this.root.querySelector(".close");H==null||H.addEventListener("click",()=>{this.state.open=!1,this.savePersisted(!1),this.render()});let R=this.root.querySelector(".form"),T=this.root.querySelector(".input");R==null||R.addEventListener("submit",b=>{b.preventDefault();let d=((T==null?void 0:T.value)||"").trim();d&&(T&&(T.value=""),this.pendingFocus=!0,this.send(d))}),this.root.querySelectorAll(".lead-form").forEach(b=>{let d=b;d.addEventListener("submit",w=>{var M;w.preventDefault();let $=new FormData(d),A={name:($.get("name")||"").toString().trim(),email:($.get("email")||"").toString().trim(),phone:($.get("phone")||"").toString().trim(),consent:($.get("consent")||"")==="on"};if(!A.name&&!A.email&&!A.phone||!A.consent)return;d.classList.add("sent");let f=d.querySelector(".lead-submit");f&&(f.textContent=n.lead.sent);let E=Number(d.dataset.i||"");if(!Number.isNaN(E)){let S=this.state.msgs[E];(M=S==null?void 0:S.actions)!=null&&M.length&&(S.actions=S.actions.filter(k=>k.type!=="lead_form"));let O=this.state.msgs.slice();O.splice(E,1),this.state.msgs=O,this.pendingScroll="bottom",this.render()}this.send(n.lead.hiddenMessage,A,n.lead.followUp,{silentUser:!0,suppressReply:!0})})}),this.applyPendingScroll(),this.applyPendingFocus()}async send(e,t,s,i){let n=this.state.session;if(n!=null&&n.token){this.state.busy=!0,i!=null&&i.silentUser||(this.state.msgs=[...this.state.msgs,{role:"user",content:e}]),this.pendingFocus=!0,this.savePersisted(!0),this.pendingScroll="bottom",this.render();try{let r=this.getApiBase(),u=await fetch(`${r}/api/conversation`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:n.token,conversationId:this.state.conversationId,externalRef:this.state.visitorSessionId,message:e,locale:this.state.locale,channel:"web",...t?{lead:t}:{}})}),h=await u.json().catch(()=>({}));if(!u.ok){let p=(h.error||"").toLowerCase();if(u.status===403&&p.includes("session expired")){let c=await this.tryRefreshSessionToken();if(c){let g=await fetch(`${r}/api/conversation`,{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({token:c,conversationId:this.state.conversationId,externalRef:this.state.visitorSessionId,message:e,locale:this.state.locale,channel:"web",...t?{lead:t}:{}})}),v=await g.json().catch(()=>({}));if(g.ok){v.conversationId&&(this.state.conversationId=v.conversationId);let y=z(v.actions),I=t?y==null?void 0:y.filter(L=>L.type!=="lead_form"):y;i!=null&&i.suppressReply||(this.state.msgs=[...this.state.msgs,{role:"assistant",content:v.reply||"",actions:I}]),s&&(this.state.msgs=[...this.state.msgs,{role:"assistant",content:s}]),this.pendingFocus=!0,this.state.busy=!1,this.savePersisted(!0),this.pendingScroll="lastAssistantStart",this.render();return}this.state.msgs=[...this.state.msgs,{role:"assistant",content:v.error||"Error"}],this.pendingFocus=!0,this.state.busy=!1,this.savePersisted(!0),this.pendingScroll="lastAssistantStart",this.render();return}}this.state.msgs=[...this.state.msgs,{role:"assistant",content:h.error||"Error"}],this.pendingFocus=!0,this.state.busy=!1,this.savePersisted(!0),this.pendingScroll="lastAssistantStart",this.render();return}h.conversationId&&(this.state.conversationId=h.conversationId);let a=z(h.actions),m=t?a==null?void 0:a.filter(p=>p.type!=="lead_form"):a;i!=null&&i.suppressReply||(this.state.msgs=[...this.state.msgs,{role:"assistant",content:h.reply||"",actions:m}]),s&&(this.state.msgs=[...this.state.msgs,{role:"assistant",content:s}]),this.pendingFocus=!0,this.state.busy=!1,this.savePersisted(!0),this.pendingScroll="lastAssistantStart",this.render()}catch{this.state.msgs=[...this.state.msgs,{role:"assistant",content:"Network error"}],this.pendingFocus=!0,this.pendingScroll="lastAssistantStart",this.state.busy=!1,this.savePersisted(!0),this.render()}}}};function x(o){return(o||"").replaceAll("&","&amp;").replaceAll("<","&lt;").replaceAll(">","&gt;").replaceAll('"',"&quot;").replaceAll("'","&#039;")}function ee(o){return o.replace(/(https?:\/\/[^\s<]+[^<.,:;"')\]\s])/g,'<a href="$1" target="_blank" rel="noopener noreferrer">$1</a>')}function te(o){var u,h;let l=x(o||""),t=ee(l).split(/\r?\n/),s=a=>a.replace(/\*\*(.+?)\*\*/g,"<strong>$1</strong>"),i=a=>s(a.join("<br/>")),n=t.findIndex(a=>/^\s*\d+\.\s+/.test(a));if(n!==-1){let a=t.slice(0,n).filter(c=>c.trim()!==""),m=[],p=[];for(let c=n;c<t.length;c++){let g=(u=t[c])!=null?u:"";if(/^\s*\d+\.\s+/.test(g)){m.push(g.replace(/^\s*\d+\.\s+/,"").trim());continue}g.trim()!==""&&p.push(g)}return m.length===0?i(t):`
      ${a.length?`<div class="lead">${i(a)}</div>`:""}
      <ol class="list">
        ${m.map(c=>`<li>${s(c)}</li>`).join("")}
      </ol>
      ${p.length?`<div class="tail">${i(p)}</div>`:""}
    `}let r=t.findIndex(a=>/^\s*[-•]\s+/.test(a));if(r!==-1){let a=t.slice(0,r).filter(c=>c.trim()!==""),m=[],p=[];for(let c=r;c<t.length;c++){let g=(h=t[c])!=null?h:"";if(/^\s*[-•]\s+/.test(g)){m.push(g.replace(/^\s*[-•]\s+/,"").trim());continue}g.trim()!==""&&p.push(g)}return m.length===0?i(t):`
      ${a.length?`<div class="lead">${i(a)}</div>`:""}
      <ul class="list">
        ${m.map(c=>`<li>${s(c)}</li>`).join("")}
      </ul>
      ${p.length?`<div class="tail">${i(p)}</div>`:""}
    `}return i(t)}customElements.get("aliigo-widget")||customElements.define("aliigo-widget",F);})();
